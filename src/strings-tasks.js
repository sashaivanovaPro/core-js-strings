...
function removeFirstOccurrences(str, substr) {
  ...
  if (position !== -1) {
    ...
    const end = str.substring(position + substr.length);
    return start + end;
  }
  if (position === -1) {
    return str;
...
function removeLastOccurrences(str, substr) {
  const position = str.lastIndexOf(substr);
  if (position !== -1) {
    const start = str.substring(0, position);
    const end = str.substring(position + substr.length);
    return start + end;
...
function countVowels(str) {
  ...
  return str
    .toLowerCase()
    .split('')
    .reduce((acc, cur) => (vowels.includes(cur) ? acc + cur : acc), '').length;
}
...
function isPalindrome(str) {
  const rep = /[\W_]/g;
  const rightStr = str.toLowerCase().replace(rep, '');
  const revertStr = rightStr.split('').reverse().join('');
  return rightStr === revertStr;
}
...
function reverseWords(str) {
  ...
  const newStr = [];
  for (let i = 0; i < strSplit.length; i += 1) {
    newStr.push(strSplit[i].split('').reverse().join(''));
  }
  return newStr.join(' ');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < letters.length; i += 1) {
    if (letters[i] === ' ') {
      ...
      const init = input.indexOf(letters[i]);
      if (init < 0) {
        newStr.push(letters[i]);
      }
      const final = output[init];
      newStr.push(final);
    }
  }
  return newStr.join(``);
